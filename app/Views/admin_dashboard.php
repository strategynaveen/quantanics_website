            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <textarea name="rejection_reason_val" id="rejection_reason_val" cols="30" rows="10"
                        ...
                        Placeholder="Enter Rejection Reason."  class="inputtag" >
                    </textarea>
...
<script>
    ...
    function fetch_user_data() {
        $.ajax({
            url: "<?php echo base_url() ?>public/index.php/Admin_controller/fetch_user_data",
            method: "POST",
            dataType: "json",
            ...
            success: function (res) {
                ...
                res.forEach(function (item) {
                    ...
                    ele = ele.add('<div class="row";>'
                        ...
                        + '<div class="col-lg-1 col-md-1 col-sm-1"><p style="font-weight: bold;">:</p></div>'
                        + '<div class="col-lg-6 col-md-6 col-sm-6""><p>' + item.domain_name + '</p></div>'
                        + '</div>'
    ...
    $(document).on('click','.click_get_val',function(event){
        ...
        var domain_txt = $('.custome_drp_text_content:eq('+get_findex+')').attr('data-dtext');
        
        // text getting function
        // alert(get_text);
        //var final_text = "";
        
        $('#selected_val_custome_drp_txt').text(domain_txt);
    ...
    // message retrive function
    function get_message_data(){
        // alert('ji');
        var message_txt = $('#selected_val_custome_drp_txt').attr('data-final-res');
        var user_id = "<?php echo  $session->get('admin_id'); ?>";
        console.log("after select domain function call");
                    ...
                    res.forEach(function(item){
                        ...
                        if (item.messager_status==="intern") {
                            if (item.mprofile==="empty") {
                                profile_url = "<?php echo base_url(); ?>public/public/uploads/common_profile.png";
                            }else{
                                profile_url = "<?php echo base_url(); ?>public/public/uploads/"+item.mid+"/"+item.mprofile;
                            }
                        }else{
                            profile_url = "<?php echo base_url(); ?>public/public/uploads/admin_profile.png";
                        }
                        // var check_if_admin = item.profile_name;
                        if (item.mid === user_id) {
                            element = element.add('<li class="send"><img src="'+profile_url+'" alert="username"><p>' + item.msg + '</p></li>')
 
                        }else{
                            element = element.add('<li class="replies"><img src="'+profile_url+'" alert="username"><p>' + item.msg + '</p></li>')
                        }
    ...
    function send_msg(){
        var user_id = "<?php echo  $session->get('admin_id'); ?>";
        var profile = "admin";
        var domain = $('#selected_val_custome_drp_txt').attr('data-final-res');
        var admin_name = "<?php echo  $session->get('admin_name'); ?>";
        var msg = $('#message').val();
        $.ajax({
            url:"<?php echo base_url('public/index.php/Admin_controller/send_message'); ?>",
            method:"POST",
            data:{
               emp_id:user_id,
               profile:profile,
            ...
            dataType:'JSON',
            success:function(res){
                console.log("Admin message send");
                console.log(res);
                get_message_data();
                $('#message').val('');
            },
            error:function(er){
                console.log("Sorry Try Again... admin send message");
                console.log(er);
            }
        })
    ...
    $(document).on('click','.msg_send_click',function(event){
        event.preventDefault();
        send_msg();
    });

    // function load domain selection drp chartbox
    cartbox_domain();
    function cartbox_domain(){
        $.ajax({
            url:"<?php echo base_url('public/index.php/Admin_controller/load_domains'); ?>",
            method:"POST",
            dataType:"JSON",
            success:function(res){
                console.log("load chartbox domains");
                console.log(res);
                $('#load_chart_domains').empty();
               
                res.forEach(function(item){
                    var element = $();
                    element = element.add('<div class="custome_drp_div click_get_val" style="">'
                    ...
                    +'<div class="custome_text_div" style="">'
                        +'<p style="margin-block:auto;" class="custome_drp_text_content" data-dtext="'+item.domain_name+'"  data-txt="'+item.domain_id+'">'+item.domain_name+'</p>'
                    +'</div>'
                    +'</div>');
                    $('#load_chart_domains').append(element);
                });
            },
            error:function(er){
                console.log("load chartbox domains error ajax");
            }
        });
    }
</script>
